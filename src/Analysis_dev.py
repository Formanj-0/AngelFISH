...
def draw_spot_arrow(ax, x, y, offset=-5, color='magenta'):
    ...
    """
    ax.arrow(x + offset, y, -offset, 0, head_width=3,
             color=color, length_includes_head=True)


class Spot_ClusterDetection_Confirmation(Analysis):
    ...
    ###########################################################################
    # Choose the FOV / cell such that the cell definitely has a TS (if possible)
    ###########################################################################
    ...
    def _display_zoom_on_cell(self, spotChannel, cytoChannel, nucChannel):
        ...
        for _, tsrow in cell_TS.iterrows():
            ...
            # If "size" is stored in the cluster DF, e.g. "cluster_size" or "nSpotsInCluster", adapt accordingly:
            cluster_size = getattr(tsrow, 'cluster_size', 1)  # fallback
        ...
        for _, frow in cell_foci.iterrows():
            ...
            # You could annotate size if desired:
            # axs[1].text(sx+8, sy, "...", color='cyan')
    ...
    def _display_zoom_on_one_spot(self, spotChannel):
        ...
        if self.chosen_ts is not None:
            ...
            if (0 <= rx < (x2 - x1)) and (0 <= ry < (y2 - y1)):
                ...
                csize = getattr(self.chosen_ts, 'cluster_size', 1)
    ...
    def _display_snr_thresholds(self, spotChannel, cytoChannel, nucChannel, thresholds=[0, 2, 3, 4]):
        ...
        for i, thr in enumerate(thresholds):
            ...
            # (the demonstration of "multiple threshold logic" is a bit contrived)
            ...
            for _, spot in below_spots.iterrows():
                sx = spot['x_px'] - dx
                sy = spot['y_px'] - dy
                draw_spot_circle(axL, sx, sy, radius=4, color='red')
    ...
    def _display_ts_barplot_by_time(self):
        ...
        # Count how many TS per (time, h5_idx, fov, cell_label)
        ts_count_df = (ts_df.groupby(['time','h5_idx','fov','cell_label'])
                           .size()
                           .reset_index(name='ts_count'))

        # Merge with all possible cells to ensure cells with 0 TS appear
        all_cells = self.cellprops[['h5_idx','fov','cell_label','time']].drop_duplicates()
        merged = pd.merge(all_cells, ts_count_df,
                          on=['h5_idx','fov','cell_label','time'],
                          how='left')
    ...
    ###########################################################################
    # 12) Optional overview plots: add new TS distribution bar-plot
    ###########################################################################
